...
uint32	getlocalip(void)
{
	...
	for (i = 0; i < DHCP_RETRY; i++) {
	    udp_sendto(slot, IP_BCAST, UDP_DHCP_SPORT,
						(char *)&dmsg_snd, len);
		// kprintf("Attempt %d: message sent\n", i);
	    ...
	    for (j=0; j<3; j++) {
		kprintf("Attempting receiving DHCP offer...\n");
		inlen = udp_recv(slot, (char *)&dmsg_rvc,
				    sizeof(struct dhcpmsg), 2000);
		if (inlen == TIMEOUT) {
			// kprintf("Attempt %d: TIMEOUT\n", i);
			continue;
		...
		// kprintf("Attempt %d: valid response\n", i);
